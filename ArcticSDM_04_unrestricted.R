...

{
  ...
  out_wd <- glue::glue("{root}Results/SDM_biasCorrection/unconstraint")
}
...
load(glue::glue("{sdm_wd}/{sp}/{sp}_modelTab.rds"))
...
if(!file.exists(glue::glue("{out_wd}/{sp}_binArray_Unconstraint.rda"))) {
  ...
  save(binarArray, file = glue::glue("{out_wd}/{sp}_binArray_Unconstraint.rda"))
