...
#   gbif_data <- "/bioing/data/ArcticSDM_data/"
#   map_data  <- "/bioing/data/ArcticSDM_data/"
#   env_data  <- "/bioing/data/ArcticSDM/environment/CHELSA data 5 km/Modern/"
#   
#   out_wd    <- "/biodiv/user/slisovsk/ArcticSDM_Results/"
# }
...
### Restricted predictions
{
  if(!file.exists(glue::glue("{out_wd}/{sp}/{sp}_MaxEnt_calibration_restricted.tif"))) {
    ...
    env_var <- predTab_curr %>% dplyr::select(names(modelTab)[-c(1:6)]) %>% 
      st_rasterize(st_as_stars(st_bbox(env_curr), 
                               nx = st_dimensions(env_curr)$x$delta, 
                               ny = st_dimensions(env_curr)$x$delta, values = NA_real_)) %>%
      rast() %>% setNames(names(modelTab)[-c(1:6)])
